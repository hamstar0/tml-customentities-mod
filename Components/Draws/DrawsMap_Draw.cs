...
namespace CustomEntities.Components {
	public partial class DrawsOnMapEntityComponent : CustomEntityComponent {
		...
		public static void DrawToFullMap( SpriteBatch sb, Texture2D tex, Color color, Vector2 worldCenter, bool isZoomed, float scale ) {
			float myScale = ( isZoomed ? Main.mapFullscreenScale : 1f ) * scale;

			int wldX = (int)worldCenter.X - (int)( (float)tex.Width * scale * 8 );
			int wldY = (int)worldCenter.Y - (int)( (float)tex.Height * scale * 8 );

			var wldRect = new Rectangle( wldX, wldY, tex.Width, tex.Height );
