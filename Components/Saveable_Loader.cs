	...
	public partial class SaveableEntityComponent : CustomEntityComponent {
		...
		protected class MyStaticInitializer : StaticInitializer {
			protected override void StaticInitialize() {
				...
				if( Main.netMode != 1 ) {
					Promises.AddValidatedPromise<PromiseArguments>( CustomEntitiesWorld.LoadValidator, ( _ ) => {
				...
				if( Main.netMode != 1 ) {
					Promises.AddValidatedPromise<PromiseArguments>( CustomEntitiesWorld.SaveValidator, ( _ ) => {
				...
				try {
					if( !SaveableEntityComponent.LoadAll( true ) ) {
						if( mymod.Config.DebugModeNetInfo ) {
							LogHelpers.Warn( "Load (json) failed." );
						}
					}
					if( !SaveableEntityComponent.LoadAll( false ) ) {
						if( mymod.Config.DebugModeNetInfo ) {
							LogHelpers.Warn( "Load (no json) failed." );
						}
					}
				} catch( Exception e ) {
